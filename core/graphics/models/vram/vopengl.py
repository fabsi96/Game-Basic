...
from glm import *
from glm.gtc.matrix_transform import rotate, scale, translate
from numpy import identity

from core.graphics.models.ram.rawobject import RawObject
from core.graphics.dataloader.texture.texture import Texture
...
class VOpenGL:
    """ Summary
       Basic class of ALL graphics graphics objects
    """
    RENDER_NONE = "NONE"
    ...
    def __updateTransformationMatrix(self) -> None:
        try:
            translationMatrix = translate(tmat4x4(), self._position)

            xRotationMatrix = rotate(tmat4x4(), radians(self._xDegrees), vec3(1.0, 0.0, 0.0))
            yRotationMatrix = rotate(tmat4x4(), radians(self._yDegrees), vec3(0.0, 1.0, 0.0))
            zRotationMatrix = rotate(tmat4x4(), radians(self._zDegrees), vec3(0.0, 0.0, 1.0))
